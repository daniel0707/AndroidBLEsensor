...
public class BleActivity extends Activity implements DeviceListFragment.OnDeviceListFragmentInteractionListener {
	...
	private static float luxometer;
	public static String deviceId;

	private MQTT mMQTT = new MQTT(this);
	private Timer timer;
	...
	public void onDeviceListFragmentInteraction(String macAddress) {
		String[] octs = macAddress.split(":");
		deviceId = "";
		for(String oct: octs)
			deviceId = deviceId+oct;
		Message msg = Message.obtain(null, BleService.MSG_DEVICE_CONNECT);
		if (msg != null) {
			msg.obj = macAddress;
			try {
				mService.send(msg);
				IMqttActionListener listener = MQTT.cretateMQTTListener();
				mMQTT.createMQTTClient(deviceId, listener);
	...
	private class SendTimerTask extends TimerTask {
		...
		public void run() {
			...
			String messageData = "{ \"d\": {" +
					"\"temperature\":" + BleActivity.temperature + ", " +
					"\"humidity\":" + BleActivity.humidity + ", "+
					"\"luxometer\":" + BleActivity.luxometer + ", ";
			try {
				// create ActionListener to handle message published results
				IMqttActionListener listener = MQTT.cretateMQTTListener();
				mMQTT.publishMQTT(messageData, listener);
